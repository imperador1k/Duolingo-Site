  ...
  if (!article) {
    return (
      <div className="blog-container" style={{ textAlign: 'center', padding: '100px' }}>
        ...
        <Link href="/blog" style={{ color: 'var(--blog-green)', fontWeight: '700' }}>Voltar ao Blog</Link>
      </div>
    );
  }
  ...
  return (
    <div className="blog-container" style={{ maxWidth: '800px' }}>
      ...
      <header style={{ textAlign: 'center', marginBottom: '60px' }}>
        <span className="category-badge" style={{ backgroundColor: 'var(--pastel-blue)', color: '#007bb5' }}>{article.category}</span>
        <h1 className="blog-title" style={{ fontSize: '3rem', marginTop: '16px', marginBottom: '16px', lineHeight: 1.1 }}>
          {article.title}
        </h1>
        <p style={{ color: 'var(--blog-gray)', fontWeight: '600' }}>{article.date} • Por {article.author}</p>
        
        <div style={{ marginTop: '40px', borderRadius: '32px', overflow: 'hidden', border: '2px solid var(--blog-border)', backgroundColor: '#f9f9f9' }}>
          <img src={article.image} alt={article.title} style={{ width: '100%', maxHeight: '400px', objectFit: 'contain', display: 'block', padding: '20px' }} />
        </div>
      </header>
      ...
      <section style={{ borderTop: '2px solid var(--blog-border)', paddingTop: '60px', marginBottom: '80px' }}>
        <h3 className="blog-title" style={{ fontSize: '1.8rem', marginBottom: '32px' }}>Continuar a ler</h3>
          ...
          {BLOG_ARTICLES.filter(a => a.slug !== slug).slice(0, 3).map((a) => (
            <Link href={`/blog/${a.slug}`} key={a.slug} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className="blog-card" style={{ padding: '16px' }}>
                <img src={a.image} alt={a.title} style={{ width: '100%', height: '100px', objectFit: 'contain' }} />
                <h5 className="blog-title" style={{ fontSize: '0.95rem', marginTop: '12px' }}>{a.title}</h5>
              </div>
            </Link>
          ))}
        </div>
