...
export default function BlogHome() {
  ...
  const categories = ["Todos", ...Array.from(new Set(BLOG_ARTICLES.map(a => a.category)))];
    ...
    <div className="blog-container">
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: '40px', marginBottom: '60px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '300px' }}>
          <span className="category-badge" style={{ backgroundColor: 'var(--pastel-orange)', color: '#a35d00' }}>{featuredArticle.category}</span>
          <h1 className="blog-title" style={{ fontSize: '3.5rem', marginBottom: '20px', lineHeight: 1.1 }}>{featuredArticle.title}</h1>
          <p style={{ fontSize: '1.2rem', color: 'var(--blog-gray)', marginBottom: '32px' }}>{featuredArticle.summary}</p>
          <Link href={`/blog/${featuredArticle.slug}`}>
            <button className="btn-ver-mais">Ler Artigo Principal</button>
          </Link>
        </div>
        <div style={{ flex: 1, minWidth: '300px' }}>
      ...
      <section style={{ marginBottom: '40px' }}>
        <div className="search-container">
          ...
          <input 
            type="text" 
            placeholder="Procurar artigos, guias ou novidades..." 
      ...
      <section>
        <h2 className="blog-title" style={{ fontSize: '2rem', marginBottom: '32px' }}>
          {searchQuery ? `Resultados para "${searchQuery}"` : "Explorar tudo"}
        </h2>
          ...
          {filteredArticles.length > 0 ? filteredArticles.map((article) => (
            <Link href={`/blog/${article.slug}`} key={article.slug} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className="blog-card" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                <img src={article.image} alt={article.title} style={{ width: '100%', height: '180px', objectFit: 'contain', marginBottom: '16px' }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <span className="category-badge" style={{ backgroundColor: '#f0f0f0', color: '#666', margin: 0 }}>{article.category}</span>
                  <span style={{ fontSize: '0.8rem', color: '#999' }}>⏱️ 3 min read</span>
                </div>
                <h3 className="blog-title" style={{ fontSize: '1.4rem' }}>{article.title}</h3>
                <p style={{ color: 'var(--blog-gray)', flex: 1 }}>{article.summary}</p>
      ...
      <section className="newsletter-box">
        <div style={{ flex: 1 }}>
          <h2 className="blog-title" style={{ color: '#856404', fontSize: '2.2rem' }}>Fica a par de tudo!</h2>
          <p style={{ color: '#a37d00', fontSize: '1.1rem' }}>Junta-te a 50,000+ alunos que recebem as nossas dicas semanais de fluência.</p>
